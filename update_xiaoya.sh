...
usage(){
  ...
  echo "-p PORT1     管理界面端口，默认： 6969"
  echo "-P PORT2     小雅AList端口，默认：5944"
...
echo -e "\e[36m端口映射：\e[0m $PORT1: 6969  $PORT2:80"

echo -e "\e[33m默认端口变更为 6969\e[0m"
...
docker run -d -p $PORT1: 6969 -p $PORT2:80 -e ALIST_PORT=$PORT2 -e MEM_OPT="$MEM_OPT" -v "$BASE_DIR":/data --restart=always --name=xiaoya-tvbox haroldli/xiaoya-tvbox:${TAG}
